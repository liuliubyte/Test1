// ==UserScript==
// @name         学习通全能助手 - 提取+客观题抢救+主观题精答 (智谱终极稳定版)
// @namespace    http://tampermonkey.net/
// @version      6.1
// @description  智谱GLM驱动，专属Hutch设计，增加强制UI渲染防消失机制，并恢复完工弹窗提示
// @match        *://*.chaoxing.com/*
...
(function() {
    ...
    // 【请在这里填入你的 智谱 API Key】
    // ==========================================
    const API_KEY = "这里填入你的智谱_API_KEY";

    // ==========================================
    // 核心 UI 挂载函数（附带防丢失机制）
    // ==========================================
    function initZtUI() {
        // 如果面板已经存在，或者网页主体还没加载出来，就退出等下一次重试
        if (document.getElementById('zt-main-panel')) return; 
        ...
        function askAI(promptText, isJsonMode = false) {
            return new Promise((resolve, reject) => {
                ...
                GM_xmlhttpRequest({
                    ...
                    onerror: function() {
                        reject("网络请求被拦截或失败，请检查 @connect 配置");
                    }
                });
            });
        }
        ...
        btnExt.onclick = function() {
            ...
            logMsg(`✅ 成功抓取 ${allQuestionElements.length} 道题！请按 F12 查看。`, 'success');
            // 【恢复弹窗】
        ...
        btnObj.onclick = async function() {
            if (API_KEY === "这里填入你的智谱_API_KEY" || API_KEY === "") {
            ...
            logMsg(`🎯 收集到 ${aiIndex - 1} 道客观题，正在呼叫智谱...`, 'info');
...
${objQuestionsText}`;
        ...
        btnSub.onclick = async function() {
            ...
            logMsg(`✍️ 锁定 ${subQuestions.length} 道主观题，采用逐题模式。`, 'success');

            let answeredCount = 0;
            for (let i = 0; i < subQuestions.length; i++) {
                let q = subQuestions[i];
                logMsg(`⌛ 正在呼叫智谱思考第 ${i + 1}/${subQuestions.length} 题...`, 'info');
    ...
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        ...
        document.addEventListener('DOMContentLoaded', initZtUI);
        window.addEventListener('load', initZtUI);
    }
    
    // 2. 终极保底：每秒扫描一次网页主体，只要加载出来了就强行挂载
    let fallbackTimer = setInterval(() => {
        if (document.body && !document.getElementById('zt-main-panel')) {
            initZtUI();
        } else if (document.getElementById('zt-main-panel')) {
